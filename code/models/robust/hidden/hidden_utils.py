...
from .hidden_options import HiDDenConfiguration, TrainingOptions
...
def save_images(original_images, watermarked_images, epoch, folder, resize_to=None):
    ...
    torchvision.utils.save_image(stacked_images, filename, format="png", nrow=original_images.shape[0], normalize=False)
...
def model_from_checkpoint(hidden_net, checkpoint):
    ...
    enc_dec_state_dict = checkpoint['enc-dec-model']

    # Create temporary state dicts for each module
    encoder_state_dict = {k.replace('encoder.', ''): v for k, v in enc_dec_state_dict.items() if k.startswith('encoder.')}
    noiser_state_dict = {k.replace('noiser.', ''): v for k, v in enc_dec_state_dict.items() if k.startswith('noiser.')}
    decoder_state_dict = {k.replace('decoder.', ''): v for k, v in enc_dec_state_dict.items() if k.startswith('decoder.')}
    hidden_net.encoder.load_state_dict(encoder_state_dict)
    hidden_net.noiser.load_state_dict(noiser_state_dict)
    hidden_net.decoder.load_state_dict(decoder_state_dict)
