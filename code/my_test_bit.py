...
def main():
    ...
    for image_id, val_data in enumerate(val_loader):
        img_dir = os.path.join('bits_test',opt['name'])
        util.mkdir(img_dir)
        ...
        for i in range(t_step):
            ...
            rb_img = util.tensor2img(visuals['RB'])
            lrgt_img = []
            for j in range(n):
            ...
            save_img_path = os.path.join(img_dir,'{:d}_{:d}_{:s}.png'.format(image_id, i, 'RB'))
            util.save_img(rb_img, save_img_path)
            ...
            psnr_rb = cal_pnsr(rb_img, gt_img)
            psnr_h = []
            for j in range(n):
            ...
            avg_psnr_rb += psnr_rb
            avg_psnr_rb_roi += calculate_psnr_roi(rb_img, gt_img)
            for j in range(n):
    ...
    avg_psnr_rb = avg_psnr_rb / idx
    avg_psnr_rb_roi = avg_psnr_rb_roi / idx
    ...
    print('# Validation # PSNR_RB: {:.4e}, PSNR_RB_ROI: {:.4e}'.format(avg_psnr_rb, avg_psnr_rb_roi))
